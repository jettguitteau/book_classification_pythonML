...
\author{Nahasat Nibir \\ DSTI School of Engineering / Data Wrangling with SQL}
\date{June 2026}
...
\begin{itemize}
    ...
    \item Capped \texttt{num\_pages} at the 99th percentile (1174.7) to reduce the influence of extreme outliers.
    \item Dropped rows with missing \texttt{average\_rating}.
\end{itemize}
After cleaning, the dataset contained 11,117 books.
...
\texttt{num\_pages, ratings\_count, text\_reviews\_count, log\_ratings\_count, log\_text\_reviews\_count, \\ reviews\_per\_rating, publisher\_te, primary\_author\_te}.
...
Lasso             & 0.3502 & 0.3253 & 0.0000 \\
Random Forest     & 0.2864 & 0.2857 & 0.2281 \\
LightGBM          & 0.2877 & 0.2848 & 0.2330 \\
CatBoost          & 0.2904 & 0.2813 & 0.2517 \\
XGBoost           & 0.2905 & 0.2853 & 0.2303 \\
\textbf{Ensemble (RF+LGBM)} & – & \textbf{0.2841} & \textbf{0.2369} \\
\bottomrule
\end{tabular}
\end{table}

The simple average ensemble of Random Forest and LightGBM achieved the lowest test RMSE (0.2841) and was selected as the final model.
...
\begin{itemize}
    ...
    \item \textbf{Batch CSV Upload}: users upload a CSV file with the same columns as the original dataset. The app validates the schema, runs predictions on all rows, and provides a downloadable CSV with an additional \texttt{predicted\_rating} column.
\end{itemize}
...
%---------------------------------------------------------------------
\section{Conclusion}
We successfully built a machine learning pipeline that predicts book ratings with a test RMSE of 0.284 and an R\textsuperscript{2} of 0.237. The ensemble of Random Forest and LightGBM provided the best performance, and the model was deployed as an interactive Streamlit application.
...
The full code, trained model, and instructions are available on GitHub: \url{https://github.com/Nibir1/Book_Rating_Prediction_ML_Pipeline}.
